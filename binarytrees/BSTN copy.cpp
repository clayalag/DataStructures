...
/*devuelve un puntero al nodo sucesor del nodo especificado.
     * Si el nodo n tiene subarbol derecho, el sucesor es el nodo
     * más a la izquierda de el subarbol derecho. Si el nodo n NO
     * tiene subarbl derecho, el sucesor es el último nodo en el
     * que hubo que seguir el puntero left para llegar a n.
     * Ej. el sucesor de 23 es 50, mientras que el sucesor de 14 es 17
    */

BTNode* BSTN::successor(BTNode *n) const {
    return successor(n, root);
}

BTNode* BSTN::successor(BTNode *n, BTNode *r) const {
    if (!n) return NULL;
    if (n->right) {
        BTNode *suc = n->right;
...
//devuelve la altura del árbol. Para el algoritmo de la figura, sería 4
int BSTN::height() const {
    return height(root);
}

int BSTN::height(BTNode *r) const {
    int leftH = -1;
    int rightH = -1;
    if (!r) return 0;
    if (r->left) leftH = height(r->left);
    if (r->right) rightH = height(r->right);
    if (leftH > rightH) return (leftH+1);
    else return (rightH+1);

}

//devuelve la cantidad de hojas del árbol. Para el algoritmo de la figura, sería 3.
int BSTN::leaves() const {
    return leaves(root);
}

int BSTN::leaves(BTNode *r) const {
    if (!r) return 0;
    if (!r->left && !r->right) return 1;
    return leaves(r->left) + leaves(r->right);
}

//devuelve el valor de la hoja con el más alto valor. Para el algoritmo de la figura, sería 67.
int BSTN::maxValueLeaf() const {
    return maxValueLeaf(root);
}

int BSTN::maxValueLeaf(BTNode *r) const {
    if (!r) return 0;
    if (!r->right) return r->key;
    return maxValueLeaf(r->right);
}
